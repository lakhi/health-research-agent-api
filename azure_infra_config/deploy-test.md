# Deployment & Testing — HeX-GiG Agent API
...
### HeX-GiG Agent (`hex-gig-agent-api`, resource group `healthsociety`)
...
changed (e.g. re-running with identical values):
   ...
   --template-file azure_infra_config/hex_gig_agent/hex-gig-agent-api.bicep \
   --parameters azure_infra_config/hex_gig_agent/hex-gig-agent-api.bicepparam \
   --mode Incremental
...
### Pause / Unpause HeX-GiG (`healthsociety`, `socialeconpsy` subscription)

HeX-GiG is paused when not in active use to reduce costs (~$135/month saving). All data and images
...
The hex_gig agent (`hex_gig_agent`) has a daily budget enforcement system to control Azure OpenAI costs.
...
1. **Pre-check**: Before each `hex_gig_agent` request, the system checks if the daily budget is exceeded
...
CREATE INDEX ix_daily_agent_usage_date ON daily_agent_usage(date);
```
...
```sh
psql -v target_date="'29-Jan-2026'" -v daily_budget=2.0 -f scripts/sql/get_daily_agent_usage.sql
```

# Must be injected BEFORE the per-agent instruction blob so it is not outweighed by the
# Simple Language agent's brevity ruleset ("max 3 paragraphs", "remove anything not asked").
COMPLETENESS_INSTRUCTION = (
    "List every outcome level the catalogue gives, including the least severe / mildest one. "
    "If your answer omits any level, even the mildest, rewrite it to include them all before sending."
)

SHARED_MARHINOVIRUS_INSTRUCTIONS: list[str] = [
    "When the knowledge base describes a consequence as a possible or occasional additional outcome (e.g. 'can also sometimes lead to...'), preserve that conditional framing in your response. Do not present conditional outcomes as guaranteed accompaniments of another tier.",
    "If asked about death or fatal outcomes, respond that you are not able to lose all 100 fitness points by simply contracting the virus, and that the maximum loss of fitness points from marhinitis is 80 points. Do not use the words 'death', 'die', 'dies', 'dying', 'died', 'fatal', or 'fatality'. Do not add any other statements.",
    "When answering general questions about the virus or its consequences, do not mention the maximum possible fitness point loss or what happens in the absolute worst case. Only include this information when the user explicitly asks about death or fatal outcomes.",
]
